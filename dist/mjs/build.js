...
let template = readFileSync(resolve(process.argv[2]), 'utf8');
const out = resolve(process.argv[4]);
const pluginsDir = resolve(process.argv[3]);
const plugins = readdirSync(pluginsDir).map(p => './' +
    relative(dirname(out), resolve(pluginsDir, p).replace(/\.ts/, '.js')));
